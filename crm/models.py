...
class Customer(models.Model):
    ...
    def show_class(self):
        return '  /  '.join([str(cla) for cla in self.class_list.all()])
    ...
    def show_status(self):
        ...
        return '<span style="color:%s">%s</span>' % (status_dic[self.status], self.get_status_display())
...
class ClassList(models.Model):
    ...
    def __str__(self):
        return '%s第%s期(%s)' % (self.course, self.semester,self.campuses)
...
class Enrollment(models.Model):
    ...
    def show_contract_approved(self):
        contract_approved_dic = {
            True:'green',
            False:'red'
        }

        return '<span style="color:%s">%s</span>'%(contract_approved_dic[self.contract_approved],self.get_contract_approved_display())
