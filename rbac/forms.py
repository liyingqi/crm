...
ICON_CHOICE = [[i[0], mark_safe(i[1])] for i in [
...
class MenuForm(BaseForm):
    ...
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.fields.get('icon').widget.attrs.pop('class')
        self.fields.get('icon').widget.attrs.pop('placeholder')
...
class PermissionForm(BaseForm):
    ...
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        choice_l = [(i.pk, i) for i in models.Permission.objects.filter(menu_id__isnull=False)]
        self.fields['parent'].choices  = choice_l
        self.fields['parent'].choices.insert(0,('','无'))
